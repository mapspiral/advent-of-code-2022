...
internal sealed class Puzzle7 : PuzzleBase<long>
{
    protected override long Solution1(IEnumerable<string> input)
    {
        var root = ParseStructure(input.ToArray());
        return root.DirectoriesWithSizeOf(100000, SizeOperation.Minimum).Sum(x => x.Size());
    }

    protected override long Solution2(IEnumerable<string> input)
    {
